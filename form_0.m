function [ output_args ] = form_0( psf )
%execute FORM given all psf, output=sum of weighted penalty function
%   used for:
%       performing FORM given the psfs
%       and calibrating all psfs together
%   inputs: 
%       psf on load side [gM1,gM2,gG,gQ,gammaF  or kmod_fixed]
%   outputs:
%       weighted penalty function        
    ...
    for imat=1:input.num_mat
        ...
        gM=psf(imat);
        gG=psf(input.num_mat+1);
        gQ=psf(input.num_mat+2);
            ...
            for aQ=input.aQ(imat,1):input.step:input.aQ(imat,2)
                ...
                for aG=input.aG(imat,1):input.step:input.aG(imat,2)
                    ...
                    switch input.des_eq
                        ...
                        case 'SFII'
                            kmod_fixed=psf(input.num_mat+3);
                            kmod=[kmod_fixed  
                                  kmod_fixed
                                  kmod_fixed
                                  kmod_fixed
                            ...
                            output(imat).decisive_design_eq(iaQ,iaG)=mean(find(z==max(z))); %save the design equation giving largest z (decisive design equation)
                        case 'SFI'
                            gammaF=psf(input.num_mat+3); %Global safety factor multiplying g and q
                            kmod=[input.Variable(1,7) %kmod of the load with shortest duration 
                                  input.Variable(2,7)
                                  max(input.Variable(1,7),input.Variable(2,7))];
                            z=[ (aG*xk(3)*gG+(1-aG)*aQ*xk(6)*gQ)*gM/xk(2)/kmod(1)
                                (aG*xk(3)*gG+(1-aG)*(1-aQ)*xk(7)*gQ)*gM/xk(2)/kmod(2)
                                ( aG*xk(3)*gammaF+(1-aG)*( aQ*xk(6)*gammaF+(1-aQ)*xk(7)*gammaF  )  )*gM/xk(2)/kmod(3)];
                            output(imat).decisive_design_eq(iaQ,iaG)=mean(find(z==max(z))); %save the design equation giving largest z (decisive design equation)
                        otherwise
                    ...
                    for iLC=1:5
                        ...
                        % output structure
                            ...
                            output(imat).z(iaQ,iaG)=max(z);
                            output(imat).governing_limit_state(iaQ,iaG)=iLC;
                            else
                                %nothing
                            end
                    end
